...
@Component
class SetTypeHandler(private val userStatesRepository: UserStatesRepository) : Handler {

    override fun getMessage(chatId: Long?, messageText: String?): SendMessage {
        return when {
            ("МЕНТОР!" == messageText?.toUpperCase()) -> {
                chatId?.let { userStatesRepository.save(UserState(it, States.MENTOR_DEFAULT)) }
                SendMessage(chatId, "Привіт Ментор!")
            }
            ("СТУДЕНТ!" == messageText?.toUpperCase()) -> {
                chatId?.let { userStatesRepository.save(UserState(it, States.STUDENT_DEFAULT)) }
